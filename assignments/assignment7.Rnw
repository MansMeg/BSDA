...
\item [c)] Using the models, with the specified weakly informative priors, report the expectation/mean (and 90\% uncertainty intervals) formatted in a table, comment on it and, if applicable, plot histograms for the following distributions:
  \begin{itemize}
