...
\subsection*{1. Model assessment: PSIS LOO-CV for factory data with Stan}
...
\begin{enumerate}
    \item Fit the models with Stan as instructed in Assignment~7. With the $N(100,100)$ prior for the $\mu$ and $\theta$ parameters and the $N(0,50)$ prior $\sigma$ and $\tau$ parameters. \rh{Assume a new Normal inverse Gaussian model and set the parameters as has been discussed during the lecture.}
...
\subsection*{2. Model assessment: Standard LOO-CV for factory data with Stan (1.5 points, only required for VG)}

PSIS LOO-CV approximates standard leave-one-out cross validation. To compare the two, implement standard leave one out cross validation for the separate model.

\begin{enumerate}
\item Include your code
\item Report the $\text{elpd}_\text{loo-cv}$ value for the separate model
\item Compare the value to the one obtained via PSIS-LOO.
\end{enumerate}
