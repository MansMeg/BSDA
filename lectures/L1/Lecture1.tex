...
\begin{frame}
  ...
  \begin{center}
    \includegraphics[width=0.92\linewidth]{figs/irma.png}
  \end{center}
  ...
  \href{https://discourse.mc-stan.org/t/announcing-stancon-2026-last-keynote/40958}{Systematic uncertainty visualization design}
  \end{center}
...
\begin{frame}{Quick check: uncertainty in decisions}

  \begin{itemize}
  \item Think of one decision from your own life, studies, or research where the outcome is uncertain.
  \pause
  \item What are the possible outcomes?
  \item What information would change your uncertainty?
  \pause
  \item Pair discussion: what would be the analogue of prior information, data, and a decision?
  \end{itemize}
...
\begin{frame}{Bayesian Analysis}
  ...
  \item Bayesian probability theory
    \begin{itemize}
    \item uncertainty is represented with probabilities\pause
    ...
    \item considered the problem of {\it inverse probability}\pause
      \begin{itemize}
...
\begin{frame}{Two types of uncertainty}
  ...
  \begin{itemize}
  \item Aleatoric uncertainty \uured{due to randomness}
    \begin{itemize}
    \item<1-> observations can't reduce this uncertainty
    \end{itemize}
    \vspace{\baselineskip}
  \item Epistemic uncertainty \uured{due to lack of knowledge}
    \begin{itemize}
    \item<2-> we can reduce this uncertainty with observations
...
\begin{frame}{Quick check: aleatoric or epistemic?}

  \begin{itemize}
  \item Classify each uncertainty, then compare with your neighbour:
    \begin{enumerate}
    \item the next roll of a fair die
    \item the unknown fraction of red chips in a bag
    \item tomorrow's temperature in Uppsala
    \end{enumerate}
  \pause
  \item Die roll: mainly aleatoric in this model
  \item Red-chip fraction: epistemic; sampling chips can reduce it
  \item Temperature: both; data and forecasts reduce epistemic uncertainty, but weather variation remains
  \end{itemize}
...
\begin{frame}{Probability in Bayesian Reasoning}
  ...
  \item Uncertainty and knowledge are expressed as probability distributions, why? \pause
    \item Analogy to physical randomness\pause
    \item Knowledge as coherence of bets (the Dutch book):
     \\ \uured{Example}: If you say: P(rain) = 0.7 and P(no rain) = 0.4, those add up to 1.1 (too high).\\
...
\begin{frame}{Probability in Bayesian Reasoning}
  ...
  \begin{itemize}
  \item Uncertainty and knowledge are expressed as probability distributions, why? \pause
  \item \uured{Accuracy arguments}: incoherent probabilities can be systematically worse estimates of truth
  \item \uured{Conditionalization}: update by zeroing possibilities ruled out by evidence and rescaling the rest
  \pause
  \item Everyone can have their own 'subjective' uncertainty, e.g.  $P(\text{rain tomorrow})$, $P(\text{Magdalena Andersson will be the next prime minister})$ \pause
  \item Frequency arguments can be difficult in some situations: P(other life in the universe)
  \item {\footnotesize See \href{https://plato.stanford.edu/entries/epistemology-bayesian/}{Stanford Encyclopedia of Philosophy: Bayesian Epistemology}}
  \end{itemize}
\end{frame}
...
% the theory of knowledge, especially with regard to its methods, validity, and scope, and the distinction between justified belief and opinion
  ...
  \begin{itemize}
    ...
    \pause
    \begin{itemize}
    \item Studies rational degrees of belief, or credences\pause
    \item Two core norms:
      \begin{itemize}
      \item Probabilism: beliefs at one time should obey the rules of probability\pause
      \item Conditionalization: beliefs should be updated by conditioning on new evidence\pause
      \end{itemize}
    \item e.g. unlike Popperian approaches, we can talk about $P(\text{black swan})$\pause
    \item Main debates: coherence, priors, idealization, and how evidence should change beliefs
    \item {\footnotesize Source: \href{https://plato.stanford.edu/entries/epistemology-bayesian/}{Stanford Encyclopedia of Philosophy}}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Statistical inference}
  ...
  \item Different types of unobserved quantities
    \begin{itemize}
    \item {\color{uured} potentially observed}: future observations, potential outcomes,
...
\begin{frame}{Notation}
  ...
  \begin{itemize}
    ...
    \item We often suppress known covariates/data $x$ and assumptions $h$
  \end{itemize}
...
\begin{frame}{Parameters, data, and predictions}
  ...
  \pause
  \item I can be sloppy: e.g. often implicitly condition on $x$, i.e. $p(\theta|y) = p(\theta|y, x)$
%  \pause
...
\begin{frame}{Quick check: model or likelihood?}

  \begin{itemize}
  \item Suppose $Y|\theta \sim \Bin(10,\theta)$, so
  \[
  p(y|\theta) = {10 \choose y}\theta^y(1-\theta)^{10-y}.
  \]
  \pause
  \item If $\theta=0.3$ and $Y$ is not observed, what varies?
  \pause
  \item If $y=7$ is observed and $\theta$ varies, what is this function called?
  \pause
  \item Is $L(\theta|y)$ a probability distribution over $\theta$?
  \end{itemize}
...
\begin{frame}{Model and likelihood}
  ...
  \]
  \pause
  \item The likelihood is \uured{generally not normalized} over $\theta$
  \end{itemize}
\end{frame}
...
\begin{frame}{Bayesian Inference I}
  ...
  \pause
  \item We state our uncertainty about $\theta$ or $\tilde{y}$ as \uured{probability distributions}
  \pause
  \begin{itemize}
...
\begin{frame}{Predictions}
  \begin{itemize}
  \item How to do inference on an unknown (potential) observable $\tilde{y}$? E.g. a future observation, potential outcome
  \pause
...
\begin{frame}{Quick check: updating with chips}

  \begin{itemize}
  \item A bag has an unknown fraction $\theta$ of red chips.
  \item Before computing anything, predict what happens to $p(\theta|y)$ after:
    \begin{enumerate}
    \item one red chip
    \item red, red, red
    \item red, yellow, red, yellow
    \end{enumerate}
  \pause
  \item Red observations move the posterior toward larger $\theta$.
  \item More observations usually make the posterior more concentrated.
  \item Mixed observations keep the posterior closer to intermediate values.
  \end{itemize}
...
\begin{itemize}
\item Urban pest-management experiment in apartments
\item Question: Does treatment reduce the number of captured roaches?
\pause
...
\item Exposure ($u_i$): Different apartments used traps for different numbers of days
\item Source: Gelman and Hill (2007, Ch. 8.3); \href{https://avehtari.github.io/modelselection/roaches.html}{Vehtari roaches cross-validation demo}
...
\item Posterior:
\begin{align*}
p(\alpha, \beta|y,X,u) \propto
p(\alpha)\prod_{k=1}^K p(\beta_k)\\
\prod_{i=1}^N \Poisson(y_i|u_i\exp(\alpha+x_i\T\beta))
\end{align*}
\pause
\item Predictive distribution:
\begin{align*}
p(\tilde{y}|\tilde{x},\tilde{u},y,X,u) =
\int &\Poisson(\tilde{y}|\tilde{u}\exp(\alpha+\tilde{x}\T\beta))\\
&p(\alpha,\beta|y,X,u)\,d\alpha\,d\beta
\end{align*}
...
\vspace{\baselineskip}
  \item<3-> Other parts of the art of probabilistic modeling are, for example,
    ...
    \item presentation: presenting and visualising the model and the results to the application experts
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Benefits and problems when going Bayesian}
  ...
  \item Benefits of Bayesian approach
    \begin{itemize}
    \item uncertainty and probability theory is the foundation
    ...
    \item straightforward model checking and evaluation
...
%  \end{itemize}
%  \pause
%  \item But it takes 10 h to estimate the model
%  \end{itemize}
%\end{frame}
...
\begin{frame}{Modeling nature}
  ...
  \begin{itemize}
  \item Drop a ball from different heights and measure time (data)
    \pause
    \begin{itemize}
    \item Newton
