...
\begin{frame}
  ...
  \begin{align*}
    \uncover<1->{p(\theta|y,n,M)
      &= \frac{p(y|\theta,n,M)p(\theta|M)}{p(y|n,M)}}\\[-0.5ex]
    \uncover<2->{&= \frac{\binom{n}{y} \theta^y(1-\theta)^{n-y}
      \underbrace{1}_{p(\theta|M)}}
      {\int_0^1 \binom{n}{y} \theta^y(1-\theta)^{n-y}
      \underbrace{1}_{p(\theta|M)} d\theta}}\\[-0.5ex]
  ...
  \item<2-> The prior appears in the numerator and denominator
  \item<3-> Since it is $1$, it drops out
...
\begin{frame}
  ...
  \item Normalisation term has \emph{Beta} function form
    ...
    \item when integrated over $(0,1)$
      the result can be presented with Gamma functions
