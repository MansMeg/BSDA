...
\begin{frame}
  ...
  \begin{align*}
    \uncover<1->{p(\theta|y,n,M)
      &= \frac{p(y|\theta,n,M)}{p(y|n,M)}}\\[1ex]
    \uncover<2->{&= \frac{\binom{n}{y} \theta^y(1-\theta)^{n-y}}
      {\int_0^1 \binom{n}{y} \theta^y(1-\theta)^{n-y} d\theta}}\\[1ex]
    \uncover<3->{&= \frac{1}{Z}\theta^y(1-\theta)^{n-y}}\\[1ex]
    \uncover<4->{&\propto \theta^y(1-\theta)^{n-y}}
  \end{align*}

  \begin{itemize}
  \item<1-> Uniform prior: $p(\theta|M)=1$ on $0\leq\theta\leq 1$
  \item<2-> The denominator normalizes the posterior
  \item<3-> $Z$ is constant as a function of $\theta$
  \item<4-> For inference, keep the part that depends on $\theta$
  \end{itemize}
