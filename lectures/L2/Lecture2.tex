...
\begin{frame}
  ...
  \item<3-> Start with uniform prior
    ...
    \end{align*}
  \item<4-> \uured{Discussion question}: Which parts of this expression change
    when $\theta$ changes, and which parts are fixed after observing $y$ and $n$?
  \end{itemize}
...
\begin{frame}
  \frametitle{Binomial: unknown $\theta$}
  \framesubtitle{Posterior derivation with a uniform prior}
  \small

  \begin{align*}
    \uncover<1->{p(\theta|y,n,M)
      &= \frac{p(y|\theta,n,M)p(\theta|M)}{p(y|n,M)}=\frac{p(y|\theta,n,M)}{p(y|n,M)}}\\[-0.5ex]
    \uncover<2->{&= \frac{\binom{n}{y} \theta^y(1-\theta)^{n-y}}
      {p(y|n,M)}}\\
    \uncover<3->{&= \frac{\binom{n}{y} \theta^y(1-\theta)^{n-y}}
      {\int_0^1 \binom{n}{y} \theta^y(1-\theta)^{n-y} d\theta}}\\[-0.5ex]
    \uncover<4->{&= \frac{1}{Z}\theta^y(1-\theta)^{n-y}}\\[-0.5ex]
    \uncover<5->{&\propto \theta^y(1-\theta)^{n-y}}
  \end{align*}

  \begin{itemize}
  \setlength{\itemsep}{0mm}
  \item<1-> Uniform prior: $p(\theta|M)=1$ on $0\leq\theta\leq 1$
  \item<1-> The prior appears in the numerator and denominator
  \item<1-> Since it is $1$, it drops out
  \item<4-> $Z$ is constant as a function of $\theta$
  \item<5-> For inference, keep the part that depends on $\theta$
  \end{itemize}
...
\begin{frame}
  ...
  \item Normalisation term has \emph{Beta} function form
    ...
    \end{equation*}
    \begin{itemize}
    \item Where does $B(a,b)=\frac{\Gamma(a)\Gamma(b)}{\Gamma(a+b)}$
      come from?
    \item with integers $\Gamma(n)=(n-1)!$\\
    (for large integers even this is challenging and usually
      $\log \Gamma(\cdot)$ is computed instead of $\Gamma(\cdot)$)
    \item \uured{Study recommendation}: ask an LLM to step through the standard-integral
      derivation, or work it through yourself using change of variables
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \item Python
    ...
    \end{itemize}
  \item CDFs and quantiles are numerically nontrivial, so use library functions
  \item \uured{Study recommendation}: use an LLM to dig deeper into why CDFs and
    quantiles often require numerical methods
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \setlength{\itemsep}{0mm}
  \item \uured{Discussion}: Why integrate over $\theta$ instead of plug in one estimate?
    \pause
...
\begin{frame}
  \frametitle{Posterior mean vs maximum likelihood}

  \begin{itemize}
  \item Maximum likelihood estimate
    \[
      \hat\theta_{\text{MLE}}=\frac{y}{n}
    \]
  \item Posterior mean with uniform prior
    \[
      \E[\theta|y]=\frac{y+1}{n+2}
    \]
  \item \uured{Discussion question}: Why can the posterior mean be a better
    estimate than the maximum likelihood estimate when $n$ is small?
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item A prior is \emph{conjugate} for a likelihood if the posterior
    is in the same distribution family as the prior
    \[
      \text{prior family} + \text{likelihood}
      \quad \Rightarrow \quad
      \text{same posterior family}
    \]
  ...
  \item Why useful?
    \begin{itemize}
    \item gives closed-form posteriors
    \item helps us understand Bayesian updating
    \item provides checks for simulation-based methods
    \item can sometimes allow analytic marginalization
    \end{itemize}

  \pause
  \item But conjugacy is not required: later we use simulation and Stan
    when closed forms are unavailable
  \end{itemize}
...
\begin{frame}
  ...
  \item Posterior
    ...
    \end{align*}
    \vskip -2mm
    \item<4-> \uured{Discussion question}: What do $\alpha$ and $\beta$ behave like
      in terms of prior successes/failures?
		  \item<5-> $\alpha$ and $\beta$ can be considered to be number of prior observations
		  \item<6-> Uniform prior when $\alpha=1$ and $\beta=1$
	  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Posterior
    ...
    \item \uured{Discussion question}: Why can the posterior mean be a better
      estimate than the maximum likelihood estimate $y/n$ when $n$ is small?
    \end{itemize}
    \pause
...
\begin{frame}
\frametitle{Weakly informative and ``noninformative'' priors}

\begin{itemize}
  \item Goal: use a prior that does not dominate the likelihood
  \item Common terms: vague, flat, diffuse, weakly informative
  \pause
  \item But ``flat'' does not always mean ``noninformative''
  \begin{itemize}
    \item flat in $\theta$ is not flat in $\log(\theta)$
    \item flat in $\theta$ is not flat in $\mathrm{logit}(\theta)$
  \end{itemize}
  \pause
  \item Prior choice should be checked on the scale that matters
  \item Prefer weakly informative priors when possible:
    rule out unreasonable values without forcing a narrow answer
\end{itemize}
...
\begin{frame}
\frametitle{Proper and improper priors}

\begin{itemize}
  \item A proper prior integrates to one:
  \[
    \int p(\theta)\,d\theta = 1
  \]
  \item An improper prior does not have a finite integral:
  \[
    \int p(\theta)\,d\theta = \infty
  \]
  \pause
  \item An improper prior can sometimes still give a proper posterior
  \item Example: binomial model with
    $p(\theta)\propto\theta^{-1}(1-\theta)^{-1}$
  \[
    \theta|y,n \sim \mathrm{Beta}(y,n-y)
  \]
  \item This posterior is proper only if $0<y<n$
  \pause
  \item Always check that the posterior is proper
\end{itemize}
...
data $y$ only through the value of $t(y)$.
  ...
  \item<2-> \uured{Discussion question}: What do you think is the sufficient
    statistic for $\theta$ in this model?
  \begin{align*}
      \uncover<3->{p(\theta|y)} &\uncover<3->{ \propto p(\theta) \prod^n p(y_i | \theta)} \\
      &\uncover<4->{ \propto \theta^{\alpha-1} (1-\theta)^{\beta-1} \prod^n\theta^y_i (1-\theta)^{1-y_i}} \\
      &\uncover<5->{ \propto \theta^{\alpha-1} (1-\theta)^{\beta-1} \theta^{\sum y_i} (1-\theta)^{n - \sum y_i}} \\
      &\uncover<6->{ \propto
        \theta^{\sum y_i+\alpha-1} (1-\theta)^{n-\sum y_i+\beta-1}}\\
      \uncover<7->{\text{after normalization}} &\\
      \uncover<7->{p(\theta|y,n,M)}
      &\uncover<7->{ = \text{Beta}(\theta|\alpha+\sum^n y_i,\beta+n-\sum^n y_i)}
  \end{align*}
  \uncover<8->{
...
\begin{frame}
  ...
  \frametitle{Why use the Normal distribution?}

  \begin{itemize}
  \item Many measurements are approximately continuous and symmetric
  \item Sums and averages are often approximately Normal
  \begin{itemize}
    \item this is the central limit theorem intuition
  \end{itemize}
  \pause
  \item Normal models are mathematically and computationally convenient
  \item Convenience is not a model check
  \pause
  \item Always ask:
  \begin{itemize}
    \item Is the outcome continuous?
    \item Is the distribution roughly symmetric?
    \item Are extreme values reasonably described by Normal tails?
  \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \frametitle{Central limit theorem: useful but limited}

  \begin{itemize}
  \item Under certain conditions, sums and averages of many random variables
    become approximately Normal
  \[
    \bar{y} \approx \N\left(\mu,\frac{\sigma^2}{n}\right)
  \]
  \pause
  \item This often justifies Normal approximations for averages and estimates
  \pause
  \item The central limit theorem has limits:
  \begin{itemize}
    \item it does not apply to all distributions, for example Cauchy
    \item the approximation may be poor for small $n$
    \item it may be poor for binomial data when $\theta$ is close to 0 or 1
  \end{itemize}
  \pause
  \item \uured{Discussion question}: Are we modeling the raw data, or an
    average/estimate?
  \end{itemize}
...
\begin{itemize}
  ...
  \pause
  \item The posterior is also Normal:
  \[
    \theta|y \sim \N(\mu_n,\tau_n^2)
  \]
  \pause
  \item The posterior mean is a weighted average:
  \[
    \mu_n =
    \frac{
      \frac{1}{\tau_0^2}\mu_0 + \frac{n}{\sigma^2}\bar{y}
    }{
      \frac{1}{\tau_0^2} + \frac{n}{\sigma^2}
    }
  \]
  \pause
  \item \uured{Prior mean and data mean are weighted by precision}
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Normal distribution - posterior uncertainty}
\framesubtitle{Several observations (iid)}

\begin{itemize}
  \item The posterior precision is
  \[
    \frac{1}{\tau_n^2}
    =
    \frac{1}{\tau_0^2}
  ...
  \item Interpretation:
  \begin{itemize}
    \item smaller $\tau_0^2$ means a stronger prior
    \item smaller $\sigma^2$ means more informative observations
    \item larger $n$ means more information from data
  \end{itemize}
  \pause
  \item \uured{Discussion question}: What is the sufficient statistic for
    $\theta$ here?
\end{itemize}
...
\begin{frame}
\frametitle{Normal distribution - limiting cases}
\framesubtitle{Several observations (iid)}

\begin{itemize}
  \item If $\tau_0^2=\sigma^2$, the prior has the same weight as one observation:
  \[
    \mu_n = \frac{\mu_0+n\bar{y}}{1+n}
  \]
  \pause
  \item If $\tau_0^2\rightarrow\infty$, the prior becomes very weak:
  \[
    \theta|y \approx \N\left(\bar{y},\frac{\sigma^2}{n}\right)
  \]
  \pause
  \item If $n\rightarrow\infty$, the data dominate the prior:
  \[
    \theta|y \approx \N\left(\bar{y},\frac{\sigma^2}{n}\right)
  \]
\end{itemize}
...
\begin{frame}
  ...
  \end{enumerate}
  \pause
  \item \uured{Discussion question}: Where do aleatoric and epistemic uncertainty
    appear in $\sigma^2+\tau_1^2$?
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  E(X) = V(X) = \lambda
  \]
  \item \uured{Discussion}: What would make this assumption questionable
    for observed count data?
  \pause
  \item The gamma distribution is a conjugate prior
  \pause
  \item \uured{Today:} Mostly the negative binomial is used for count data
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Poisson-Gamma model}

  \begin{itemize}
  \item Observations $y=(y_1,\ldots,y_n)$
    \begin{align*}
  ...
  \item Posterior
    \begin{align*}
      p(\lambda|y) &\propto
      \lambda^{\sum_i y_i}\exp(-n\lambda)
      \lambda^{\alpha-1}\exp(-\beta\lambda)\\
      &\propto \lambda^{\alpha+\sum_i y_i-1}
      \exp[-(\beta+n)\lambda]
    \end{align*}
  \item<3-> Thus
    \[
      \lambda|y \sim \text{Gamma}\left(\alpha+\sum_i y_i,\beta+n\right)
    \]
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Poisson-Gamma posterior predictive}

  \begin{itemize}
  \item Integrate over uncertainty in $\lambda$
    \[
      p(\tilde{y}|y)=\int p(\tilde{y}|\lambda)p(\lambda|y)d\lambda
    \]
  \pause
  \item If $\lambda|y\sim\text{Gamma}(\alpha_n,\beta_n)$, then
    \[
      \tilde{y}|y\sim\text{NegBinomial}
      \left(\text{size}=\alpha_n,\text{prob}=\frac{\beta_n}{\beta_n+1}\right)
    \]
  \pause
  \item Here
    \[
      \alpha_n=\alpha+\sum_i y_i,\qquad \beta_n=\beta+n
    \]
  \item This is the count-data analogue of posterior predictive averaging
  \end{itemize}
\end{frame}
