...
\begin{frame}{Simon Newcomb's speed of light experiment in 1882}
  ...
  Washington Monument and back, a total distance of 7442 meters.}
  \pause
  {\small
  Here $y_i$ is one coded round-trip time: measured time minus 24,800
  nanoseconds. The $y>0$ curve is a sensitivity check that removes the two
  negative coded values (not negative travel times).}
  \begin{center}
    \vspace{-0.5\baselineskip}
    {\includegraphics[width=5.2cm]{figs/newcomb_data.pdf}}\\
    \vspace{-1\baselineskip}
    \only<1>{\phantom{\includegraphics[width=5.2cm]{figs/newcomb_posterior1.pdf}}}
    \only<2>{\includegraphics[width=5.2cm]{figs/newcomb_posterior1.pdf}}
    \only<3>{\includegraphics[width=5.2cm]{figs/newcomb_posterior2.pdf}}
    \only<4>{\includegraphics[width=5.2cm]{figs/newcomb_posterior3.pdf}}
  \end{center}
...
\begin{frame}{Recap}
  ...
  \begin{itemize}
  \item Multiparameter posteriors are joint distributions.
  \item Marginalization removes nuisance parameters.
  \item Predictive distributions include parameter uncertainty and new
    observation noise.
  \item Posterior draws make derived quantities easy to summarize.
