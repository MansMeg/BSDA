...
\emph{Variance (dimension-free rate).}
If $\operatorname{Var}[h(X)] < \infty$, then
\[
\operatorname{Var}(\hat{\mu}_S)
= \operatorname{Var}\!\left(\frac{1}{S}\sum_{s=1}^S h(X^{(s)})\right)
= \frac{1}{S^2}\sum_{s=1}^S \operatorname{Var}(h(X^{(s)}))
= \frac{\operatorname{Var}[h(X)]}{S}.
\]
Thus the RMSE satisfies
\[
\mathrm{RMSE}(\hat{\mu}_S)
= \sqrt{\frac{\operatorname{Var}[h(X)]}{S}} \;\propto\; S^{-1/2},
\]
independent of the dimension of $X$.

\medskip
\emph{Consistency.}
By the strong law of large numbers, if $\mathbb{E}[\,|h(X)|\,] < \infty$,
\[
