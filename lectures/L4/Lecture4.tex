...
\frametitle{Monte Carlo integration}
  ...
  \begin{itemize}
    \item Basic idea: \uured{approximate the sum using samples}
    \begin{align*}
         {\displaystyle \int _{a}^{b}f(x)\,dx \approx \sum _{s=1}^{S}f(x_{s}^{*})\,\Delta x_{s},}
    \end{align*}
    where
...
\frametitle{Monte Carlo integration}
  ...
  \begin{itemize}
    ...
    \]
    \pause
    \item \uured{Monte Carlo}: If we have $\theta_s \sim p(\theta|y)$ we approximate
        \[
...
\uured{Variance (dimension-free rate).}
...
%\medskip
%\emph{Consistency.}
%By the strong law of large numbers, if $\mathbb{E}[\,|h(\theta)|\,] < \infty$,
%\[
%\hat{\mu}_S \;\xrightarrow{\text{a.s.}}\; \mu \qquad \text{as } S \to \infty.
%\]
