...
\frametitle{Stan tutorial videos for this week}

  \begin{itemize}
  \item Before Assignment 6, watch the short Stan tutorials:
    \begin{itemize}
    \item \href{https://www.youtube.com/watch?v=4t6niM6sksI}{Getting started with Stan in R}
    \item \href{https://www.youtube.com/watch?v=0FdMZwIbJ_4}{Have I converged? Convergence checks in Stan}
    \end{itemize}
  \item Python users may also watch
    \href{https://www.youtube.com/watch?v=YtR18hdAWmU}{Getting started with Stan in Python}
  \item While watching, look for the roles of \texttt{data},
    \texttt{parameters}, \texttt{model}, \texttt{generated quantities},
    and the main diagnostics reported by Stan
  \end{itemize}
...
\frametitle{Binomial model - Stan Playground}

  \begin{itemize}
  \item We can run the same Stan code directly in the browser:
    \href{https://stan-playground.flatironinstitute.org/}{stan-playground.flatironinstitute.org}
  \item Useful for trying Stan syntax, changing priors, checking compiler
    messages and inspecting draws without installing Stan locally
  \item For the binomial example above, use this data in \texttt{data.json}:
  \end{itemize}
...
\end{lstlisting}}
  ...
  \begin{itemize}
  \item For assignments, still submit reproducible Stan and R/Python code
    as usual
  \end{itemize}
...
\frametitle{Research front: HMC and Stan}

{\small
  \begin{itemize}
  \item Optional advanced talks after this lecture:
    \begin{itemize}
    \item \href{https://www.youtube.com/watch?v=Usjt1cK5fQc}{Bob Carpenter: Within-orbit adaptive leapfrog sampler (WALNUTS)}
    \item \href{https://www.youtube.com/watch?v=wIR1YXFrNaE}{Adrian Seyboldt: From Mass Matrix Adaptation to Normalizing Flows}
    \end{itemize}
  \item You are not expected to understand every technical detail
  \item While watching, focus on:
    \begin{itemize}
    \item What limitation of current HMC/NUTS is being discussed?
    \item What new idea is proposed?
    \item Why might this matter for difficult Bayesian models?
    \end{itemize}
  \end{itemize}
}
...
\frametitle{Extra material for Stan}
  ...
  \end{itemize}
  \item Short practical Stan tutorials
    \begin{itemize}
    \item[-] \url{https://www.youtube.com/playlist?list=PLCrWEzJgSUqwL85xIj1wubGdY15C5Gf7H}
    \end{itemize}
  \end{itemize}
\end{frame}
