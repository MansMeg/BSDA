...
\begin{itemize}
\item We want to simulate from the Hamiltonian
\begin{align}
\frac{d\theta}{dt} &=  M^{-1} \psi\\
\frac{d\psi}{dt} &=  \frac{\partial \log q(\theta|y)}{\partial \theta}
\end{align}
\item We can do this using a \uured{discrete} approximation: the leapfrog integrator
\item We take $L$ leapfrog steps with step size $\epsilon$ as
\begin{align}
\psi \leftarrow &\psi + \frac{1}{2}\epsilon \frac{d\log q(\theta|y)}{d\theta}\\
\theta \leftarrow& \epsilon M^{-1}\psi\\
\psi \leftarrow &\psi + \frac{1}{2}\epsilon \frac{d\log q(\theta|y)}{d\theta}
\end{align}
\item Discretization introduce a small error that increases with $\epsilon$
\end{itemize}
\end{frame}
