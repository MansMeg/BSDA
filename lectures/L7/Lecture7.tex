...
\frametitle{Stan tutorial for hierarchical models}

  \begin{itemize}
  \item Before Assignment 7, watch
    \href{https://www.youtube.com/watch?v=dNZQrcAjgXQ}{Hierarchical Modelling in Stan: Predicting the Premier League}
  \item While watching, connect the example to:
    \begin{itemize}
    \item partial pooling
    \item group-level parameters
    \item comparison with non-hierarchical models
    \item Stan diagnostics for hierarchical models
    \end{itemize}
  \end{itemize}
...
\frametitle{Example}
  ...
  \item Suppose we're observing coin flips, but we're not sure about the bias $\theta$
    \begin{itemize}
      \item We think the flips are 'the same kind of thing' (no reason to treat flip 1 differently from flip 2): \uured{Exchangeability}
