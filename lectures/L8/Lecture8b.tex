...
\begin{frame}{Goal of model evaluation: The decision problem}

\begin{itemize}
\item Evaluate the utility of model $M$ for new \uured{unseen data} $\tilde{y}$:
\[
u_M = \int u(\tilde{y}) p_\text{true}({\color{uured}\tilde{y}}) d{\color{uured}\tilde{y}}\,,
\]
...
\frametitle{Measures of predictive accuracy: Points predictions}
...
\begin{frame}
\frametitle{Measures of predictive accuracy: Probabilistic predictions}
...
\item But $p_\text{true}$ is (almost always) \uured{unknown}.
