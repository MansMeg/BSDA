...
% The ideas from importance sampling will come back.
...
\begin{frame}
  \frametitle{Importance sampling for LOO-CV}
   ...
   \item Having samples $\theta^s$ from $p(\theta^s|y)$
     ...
     \begin{align*}
       w_i^s=\frac{p(\theta^s|y_{-i})}{p(\theta^s|y)}\propto\frac{1}{\color{red} p(y_i|\theta^s)}.
     \end{align*}
 \pause
   \item If evaluated with $\tilde{y}_i=y_i$
     \begin{align*}
       p(y_i|y_{-i})\approx\frac{1}{\sum_{s=1}^S\frac{1}{p(y_i|\theta^s)}},
     \end{align*}
   \end{itemize}
